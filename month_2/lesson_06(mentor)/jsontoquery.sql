...
-- //  2. MGorkiy        60         75
SELECT b.name, 
	AVG(CASE WHEN bt.type='plus' THEN (bt.quantity) ELSE 0 END) as "plus",
	AVG(CASE WHEN bt.type='minus' THEN (bt.quantity) ELSE 0 END) as "minus"
	FROM branch b
...
SELECT u.name, 
	AVG(CASE WHEN bt.type='plus' THEN (bt.quantity*p.price) ELSE 0 END) as "plus",
	AVG(CASE WHEN bt.type='minus' THEN (bt.quantity*p.price) ELSE 0 END) as "minus"
	FROM "user" u
